...
class ConvAE(object):
    def __init__(self, n_input, kernel_size, n_hidden, reg_constant1=1.0, re_constant2=1.0, re_constant3=1.0,
        ...
        latent = tf.layers.dense(latent, 7, activation=None)
        z = latent

        if ds is not None:
            # pslb0 = tf.layers.dense(z, 4*ds, kernel_initializer=tf.random_normal_initializer(),activation=tf.nn.sigmoid,name='ss_d0')

            pslb = tf.layers.dense(z, ds, kernel_initializer=tf.random_normal_initializer(), activation=tf.nn.softmax,
                                   name='ss_d')
        ...
        self.x_r = tf.layers.dense(self.x_r, 3072, activation=None, name='d4')
        ...
        self.x_r2 = tf.layers.dense(self.x_r2, 3072, activation=None, name='d4', reuse=True)
        # self.x_r = self.x_r2
        ...
        self.labelloss = tf.losses.softmax_cross_entropy(onehot_labels=cluster_assignment1, logits=pslb,
                                                         weights=weight_label)

        self.graphloss = tf.reduce_sum(tf.nn.relu((1 - eq) * C) + tf.nn.relu(eq * (0.001 - C))) + tf.reduce_sum(
            tf.square(regass))

        self.loss = self.reconst_cost + reg_constant1 * self.reg_losses + re_constant2 * self.selfexpress_losses
        self.loss2 = (
                    self.reconst_cost + self.tracelossx + +re_constant2 * self.selfexpress_losses + re_constant3 * self.labelloss + re_constant4 * self.graphloss)
    ...
    def decoder(self, z, weights, shapes):
        ...
        while iter_i < n_layers:
            ...
            layer3 = tf.add(tf.nn.conv1d_transpose(layer3, weights['dec_w' + str(iter_i)], tf.stack(
                [tf.shape(self.x)[0], shape_de[1], shape_de[2], shape_de[3]]), stride=2, padding='SAME'),
    ...
    def partial_fit(self, X, lr, mode=0):  #
        cost0, cost1, cost2, cost3, summary, _, Coef, d, l = self.sess.run((self.reconst_cost, self.selfexpress_losses,
                                                                            self.selfexpress_losses2, self.tracelossx,
...
def test_face(Img, Label, CAE, num_class, lr2=5e-4):
    ...
    for i in range(0, 8 - num_class):
        ...
        label_10_subjs = np.squeeze(label_10_subjs) #竖列变成横排
        ...
        if pred == True:
            ...
            while epoch < pre_step:
                indices = np.arange(0, rawImg.shape[0]) #[0 1 2 ... 2099]
            ...
            CAE.save_model()
            pred = True
            visualize(Img, Label, CAE, './DSClustering/Results_original/Result10/tsne-ae.pdf')
        ...
        for posti in range(1):
            ...
            file_path = "./DSClustering/Results_original/Result10/"
            file_name = f"reg_{reg2}_{reg3}_{reg4}.xlsx"
            ...
            # visualize3(L, Label, CAE, 'D:/desktop/diffusion model/Results/clustering/tsne-sae%d.pdf' % figi)
            # visualize(Img, Label, CAE, 'D:/desktop/diffusion model/Results/clustering/tsne-sae%d.pdf' % figi)
            figi += 1
    acc_ = np.array(acc_)
    mm = np.max(acc_)
...
if __name__ == '__main__':
    data = hdf5storage.loadmat('./DSClustering/data/original_test_all.mat')
    Img = data['data']
    Label = data['gnd']
    print("Dataset loaded...")
    rawLabel = np.reshape(Label, (-1, 1))
    num = rawLabel.shape[0]
    rawImg = np.reshape(Img, [num, -1])
    ...
    model_path = './DSClustering/_original/Result10/model-fullfc.ckpt'
    restore_path = './DSClustering/Results_original/Result10/model-fullfc.ckpt'
    logs_path = './DSClustering/Results_original/Result10/logs'
            ...
            for reg4 in [1]:
                for lr2 in [1e-4]:
                    try:
                        ...
                        med = []
                        iter_loop = 0
                        while iter_loop < len(all_subjects): #len(all_subjects)=1
                            num_class = all_subjects[iter_loop] #5
                            batch_size = num_class * sc

                            tf.reset_default_graph()
                            CAE = ConvAE(n_input=n_input, n_hidden=n_hidden, reg_constant1=reg1, re_constant2=reg2,
                                         re_constant3=reg3, re_constant4=reg4,
                                         kernel_size=kernel_size, batch_size=batch_size, ds=num_class,
