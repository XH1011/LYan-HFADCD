...
def train_on_step(image_batch):
    image_batch = tf.reshape(image_batch, shape=[-1, 1, 3072, 1])
...
x_eval=tf.reshape(sample_cwru,shape=[-1,1,3072,1])
# print(x_eval.shape)
...
# np.savetxt('./Results/loss_ddpm.txt',loss_curve)



# #---GENERATING SAMPLES FORM DDPM----
# Load the model weights
dir='./Results/model_ddpm/model_last_999.ckpt'
...
def noised_images(x):
    ...
    x = tf.reshape(x, shape=(-1, 1, 3072, 1))
...
def generate_images_x(x,num_images=16):
    ...
    x = noised_images(x)
    samples=tf.reshape(x, shape=(-1, 1, 3072, 1))
...
# # 功能1：生成数据
#原始数据
save_faults= ['x0_train','x0_test','x1_train','x1_test','x2_train','x2_test','x3_train','x3_test',
              'x4_train','x4_test','x5_train','x5_test','x6_train','x6_test']
for save_fault in save_faults:
    data_name='./data/'+save_fault+'.pkl'
    _,save_x=load_data(file=data_name,BATCH_SIZE=32)
    # x = save_x.reshape(-1, 1, 128, 1)
    if save_fault[3:8]=='train':
       save_x_f = save_x[0:400, :]
       save_x_b = save_x[400:700, :]
       new_samples_x_train_f = generate_images_x(save_x_f, num_images=400)
       with open('./Results/data_generate/generate_'+save_fault+'_f.pkl', 'wb') as f:
            pickle.dump(new_samples_x_train_f, f, pickle.HIGHEST_PROTOCOL)
            print(save_fault)
       new_samples_x_train_b = generate_images_x(save_x_b, num_images=300)
       with open('./Results/data_generate/generate_'+save_fault+'_b.pkl', 'wb') as f:
            pickle.dump(new_samples_x_train_b, f, pickle.HIGHEST_PROTOCOL)
            print(save_fault)
    else:
       new_samples_x_test = generate_images_x(save_x, num_images=300)
       with open('./Results/data_generate/generate_' + save_fault + '.pkl', 'wb') as f:
           pickle.dump(new_samples_x_test, f, pickle.HIGHEST_PROTOCOL)
           print(save_fault)
