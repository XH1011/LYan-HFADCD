...
for j in range(1, 11):
    ...
    for k in range(11):
        ...
        for i in range(len(labels)):
            pre_labels[i] = {0: 0, 1: -1}[np.argmax([prob_n[i],prob_f[i]])]
        n_error=0
        for i in range(len(labels)):
        ...
        # df_result = pd.concat([df_result,  id_df,accuracy_df,F1_df], axis=1)
        # with pd.ExcelWriter('D:/Desktop/paper1/experiment/fusion_2-class2.xlsx', engine='openpyxl', mode='w') as writer:
...
# 将结果DataFrame写入Excel文件
        ...
        # accuracy_df = pd.DataFrame({'Accuracy': [accuracy]})
        # F1_df = pd.DataFrame({'F1_score': [F1_score]})
        df_result = pd.concat([df_result,  id_df], axis=1)
        with pd.ExcelWriter('D:/Desktop/paper1/experiment/fusion_2-class2.xlsx', engine='openpyxl', mode='w') as writer:
            df_result.to_excel(writer, sheet_name='Sheet1',  float_format='%.4f')